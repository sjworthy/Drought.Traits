...
# all data
all.data = read.csv("./New.dfs/all.data.BACI.csv", row.names = 1) # 407 data points
# annual data
annual.data = read.csv("./New.dfs/annual.data.BACI.csv", row.names = 1) # 79 data points
# all perennial data
perennial.data = read.csv("./New.dfs/perennial.data.BACI.csv", row.names = 1) # 321 data points
# grass
grass = read.csv("./New.dfs/grass.BACI.csv", row.names = 1) # 160 data points
# forbs
forb = read.csv("./New.dfs/forb.BACI.csv", row.names = 1) # 201 data points
# grass perennial
grass.perennial = read.csv("./New.dfs/grass.perennial.data.BACI.csv", row.names = 1) # 127 data points
...
# removed cover.change -79.47619 -54.15358 -38.00000 -32.70000 -30.89000 -29.79665  24.72857  28.91667

all.data.otl.rm = subset(all.data, all.data$cover.change >-29.79663 & all.data$cover.change < 24.72856)
# lose 8 data points, lose 2 species
annual.data.otl.rm = subset(annual.data, annual.data$cover.change >-29.79663 & annual.data$cover.change < 24.72856)
# lose 4 data points, lose 0 species
perennial.data.otl.rm = subset(perennial.data, perennial.data$cover.change >-29.79663 & perennial.data$cover.change < 24.72856)
# lose 4 data points, lose 2 species
grass.data.otl.rm = subset(grass, grass$cover.change >-29.79663 & grass$cover.change < 24.72856)
# lose 7 data points, lose 1 species
forb.data.otl.rm = subset(forb, forb$cover.change >-29.79663 & forb$cover.change < 24.72856)
# lose 0 data points, lose 0 species
grass.perennial.data.otl.rm = subset(grass.perennial, grass.perennial$cover.change >-29.79663 & grass.perennial$cover.change < 24.72856)
...
all.data = left_join(all.data.otl.rm, enviro.data, by="site_code") # 65 unique sites
