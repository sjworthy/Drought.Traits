...
test.data=read.csv("IDE_traitdata_Feb27.csv")


test.data$anpp=((test.data$drt.cover-test.data$amb.cover)/test.data$amb.cover)*100

test.data$cover.response.2=exp(test.data$cover.response)
test.data$log.drt.cover=log(test.data$drt.cover+1)
test.data$log.amb.cover=log(test.data$amb.cover+1)
test.data$cover.response.log=test.data$log.drt.cover-test.data$log.amb.cover
test.data$cover.response.exp=exp(test.data$cover.response.log)

test.data$scale.drt.cover=scale(test.data$drt.cover)

scale(test.data$drt.cover)-scale(test.data$amb.cover)
...
reg.tree.1=rpart(cover.response ~ Leaf.area + Seed.dry.mass, data=test.data)
...
brt.4=gbm.step(data=test.data, gbm.x = c(3,9), gbm.y=41,
...
brt.5=gbm.step(data=test.data, gbm.x = c(3,9,), gbm.y=41,
               family = "gaussian", tree.complexity = 1, learning.rate = 0.01,
               bag.fraction = 0.75, n.trees = 50, verbose = FALSE)
...
observed = test.data$cover.response
